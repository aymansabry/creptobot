...
async def process_investment_amount(message: types.Message, state: FSMContext):
    try:
        ...
        # هنا يمكنك استدعاء دالة بدء التداول الآلي
...
@dp.callback_query(StateFilter(InvestmentStates.waiting_for_report_start_date), Text(startswith="date_"))
async def handle_date_selection(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if period == "today":
        ...
        # تعيين تاريخ بداية افتراضي أو البحث في قاعدة البيانات عن أول صفقة
...
async def set_profit_percentage(message: types.Message, state: FSMContext):
    try:
        ...
        # هنا يتم حفظ القيمة الجديدة في ملف الإعدادات أو قاعدة البيانات
        # settings.PROFIT_PERCENTAGE = profit_percentage 
...
async def handle_all_callbacks(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if data == "help":
        ...
        await start_api_key_registration(callback_query, state)
        ...
        await start_investment(callback_query, state)
    elif data == "fake_investment":
        await fake_investment(callback_query)
    elif data == "account_statement":
        await ask_report_start_date(callback_query, state)
    elif data == "market_status":
        await market_status_report(callback_query)
    elif data == "stop_investment":
        await stop_investment(callback_query)
    elif data == "admin_panel":
        await cmd_admin_panel(callback_query)
    elif data == "edit_profit_percentage":
        await edit_profit_percentage(callback_query, state)
    elif data == "user_count":
        await get_user_count(callback_query)
    elif data == "online_user_count":
        # هذه الميزة تتطلب منطقًا إضافيًا لتتبع المستخدمين النشطين
        await bot.send_message(user_id, "ميزة عدد المستخدمين أونلاين تحت التطوير.")
    elif data == "investment_reports":
        # هذه الميزة تتطلب منطقًا إضافيًا لإنشاء التقارير
        await bot.send_message(user_id, "ميزة تقارير الاستثمار تحت التطوير.")
    elif data == "bot_status":
        await get_bot_status(callback_query)
    elif data.startswith("date_"):
        await handle_date_selection(callback_query, state)
    elif data == "skip_passphrase":
        await skip_passphrase(callback_query, state)
    else:
