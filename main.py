...
# تعديل DATABASE_URL اذا كان mysql:// --> mysql+pymysql://
...
def create_kucoin_clients(user: User):
    market_client = KuCoinMarket()
    trade_client = KuCoinTrade(user.kucoin_api, user.kucoin_secret, user.kucoin_passphrase)
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "أهلاً! استخدم الأوامر لربط حساباتك وبدء الاستثمار:\n"
...
async def process_kucoin_passphrase(message: types.Message, state: FSMContext):
    ...
    try:
        trade_client = KuCoinTrade(kucoin_api, kucoin_secret, kucoin_passphrase)
