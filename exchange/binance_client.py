...
class BinanceClient:
    def __init__(self, api_key: str | None = None, api_secret: str | None = None):
        opts = {'apiKey': api_key, 'secret': api_secret, 'enableRateLimit': True, 'options': {'defaultType': 'spot'}}
        # allow unauthenticated if no keys provided
        if api_key is None and api_secret is None:
            # ccxt requires at least empty strings to instantiate proper exchange object
            opts['apiKey'] = ''
            opts['secret'] = ''
        self.x = ccxt.binance(opts)
